  ...
  return (
    <header
      className={cn(
        ...
        scrolled || open ? "bg-emerald-deep/95 backdrop-blur-md shadow-[0_1px_0_rgba(201,162,39,0.25)]" : "bg-transparent",
      )}
    >
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <span className="relative block w-5 h-3.5">
              <span className={cn("absolute left-0 top-0 h-[1.5px] w-5 bg-current transition-transform duration-300", open && "translate-y-[6px] rotate-45")} />
              <span className={cn("absolute left-0 top-[6px] h-[1.5px] w-5 bg-current transition-opacity", open && "opacity-0")} />
              <span className={cn("absolute left-0 top-[12px] h-[1.5px] w-5 bg-current transition-transform duration-300", open && "-translate-y-[6px] -rotate-45")} />
            </span>
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            id="mobile-nav"
            className="lg:hidden fixed inset-0 top-16 bg-emerald-deep z-40 overflow-y-auto"
